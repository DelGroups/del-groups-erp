...
export async function fetchPurchaseById(id: string): Promise<PurchaseRecord | null> {
  ...
  return {
    ...
    is_official: purchase.is_official === true,
    contract_id: purchase.contract_id ?? null,
    vat_mode: purchase.vat_mode ?? null,
    subtotal_amount: purchase.subtotal_amount != null ? Number(purchase.subtotal_amount) : null,
    vat_rate: purchase.vat_rate != null ? Number(purchase.vat_rate) : null,
    vat_amount: purchase.vat_amount != null ? Number(purchase.vat_amount) : null,
    grand_total: purchase.grand_total != null ? Number(purchase.grand_total) : null,
