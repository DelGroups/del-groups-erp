...
function ProductFormSection({
  ...
  return (
    <Card padding={false}>
      ...
      <CardContent className="space-y-4 p-6 pt-4">{children}</CardContent>
    </Card>
  );
}
    ...
    <>
      <form onSubmit={handleSubmit} className="w-full">
        ...
        {embedded ? (
          ...
          <div className="mt-8 flex w-full items-center justify-end gap-3 border-t border-slate-200 pt-6 dark:border-slate-700">
            {onCancel ? (
              <Button type="button" variant="outline" onClick={onCancel}>
                {t("common.cancel")}
              </Button>
