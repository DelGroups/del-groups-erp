...
interface ProductBarcodePanelProps {
  ...
  /** Toolbar = single row on wide forms; stacked = vertical layout for drawers. */
...
export default function ProductBarcodePanel({
  ...
  const preview = code ? (
    <div
      className={
        isStacked
          ? "flex justify-center rounded-[var(--erp-radius-md)] border border-dashed border-[color:var(--erp-border-default)] bg-white px-4 py-3"
          : "inline-flex shrink-0 items-center justify-center rounded border border-dashed border-[color:var(--erp-border-default)] bg-white px-1.5 py-1"
      }
      aria-hidden
    >
      <BarcodeDisplay
        ...
        width={isStacked ? 1 : 0.7}
        height={isStacked ? 28 : 16}
        fontSize={isStacked ? 8 : 6}
        showValue
      />
    </div>
  ) : null;
