...
export default function ProductionContractPrintTemplate({ data }: Props) {
  ...
  const contractNo =
    resolveContractNumber({ ...(contract || {}), ...order, order_no: order.order_no }) ||
    order.order_no;
  const customerName =
    resolveContractCustomerName({ ...(contract || {}), ...order }) ||
    t("common.anonymousCustomer");
  const contractDate = resolveContractDate({
    ...(contract || {}),
    ...order,
    created_at: order.created_at,
  });
  const advancePayment = resolveContractAdvancePayment({ ...order, ...(contract || {}) });
  const projectScope = resolveProjectScope(order) || "—";
  const contractTerms = resolveContractTerms(order) || "—";
  const formattedContractDate = formatDate(contractDate) || "—";
  const formattedDeliveryDate =
    formatDate(resolveContractDeliveryDate({ ...(contract || {}), ...order })) || "—";
  ...
  return (
    <div className="mx-auto w-[210mm] bg-white p-10 font-sans text-black">
      <header className="mb-6 border-b-2 border-slate-900 pb-4 text-center">
        ...
        <h1 className="mt-1 text-2xl font-bold">{displayText(companyName)}</h1>
        <p className="mt-1 text-sm">{t("production.contract.documentTitle")}</p>
        <p className="mt-2 text-sm">
          <strong>{t("production.contract.contractNo")}:</strong>{" "}
          {displayText(contractNo)}
          {" · "}
          <strong>{t("common.date")}:</strong> {formattedContractDate}
        </p>
      </header>
      ...
      <section className="mb-5 text-sm">
        ...
        <p>
          <strong>{t("production.projectName")}:</strong> {displayText(order.project_name)}
        </p>
        <p>
          <strong>{t("production.expectedDelivery")}:</strong> {formattedDeliveryDate}
        </p>
        ...
        <table className="w-full border-collapse">
          <tbody>
            ...
            <tr>
              ...
              <td className="border p-2 text-right">
                {money(advancePayment, t("common.currency"))}
              </td>
            </tr>
      ...
      <section className="mb-8 text-sm">
        ...
        <p className="whitespace-pre-wrap leading-6">{contractTerms}</p>
      </section>
