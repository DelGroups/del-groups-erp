...
BEGIN
  ...
  v_journal_id := public.post_journal_entry(
    jsonb_build_object(
      ...
      'memo', format('Satış fakturası %s', v_doc_no),
      'lines', jsonb_build_array(
        ...
        jsonb_build_object(
          ...
          'line_memo', 'Satış gəliri — ' || v_doc_no
        )
      )
    )
  );
...
BEGIN
  ...
  v_journal_id := public.post_journal_entry(
    jsonb_build_object(
      ...
      'memo', format('Alış fakturası %s', v_invoice_number),
      'lines', jsonb_build_array(
        jsonb_build_object(
          'coa_code', '1300',
          'debit', v_total_amount,
          'credit', 0,
          'line_memo', 'Inventar — ' || v_invoice_number
        ),
        jsonb_build_object(
          'coa_code', '2100',
          'debit', 0,
