-- Repair: ensure contractor_id + production_type exist on production_orders (schema cache fix)
...
SET production_type = CASE
  ...
  ELSE COALESCE(production_type, 'internal_custom')
END
WHERE production_type IS NULL OR production_type = '';
