-- Multi-Tier Pricing: optional wholesale/distributor price overrides per
-- product, and a default price tier per customer. Retail has no separate
-- column - sell_price already is the retail/default price and is read by
-- dozens of existing call sites, so it stays the single source of truth for
-- that tier rather than being duplicated.

ALTER TABLE products
  ADD COLUMN IF NOT EXISTS price_wholesale NUMERIC,
  ADD COLUMN IF NOT EXISTS price_distributor NUMERIC;

ALTER TABLE customers
  ADD COLUMN IF NOT EXISTS default_price_tier TEXT NOT NULL DEFAULT 'retail';

ALTER TABLE customers
  DROP CONSTRAINT IF EXISTS customers_default_price_tier_check;

ALTER TABLE customers
  ADD CONSTRAINT customers_default_price_tier_check
  CHECK (default_price_tier IN ('retail', 'wholesale', 'distributor'));

-- Extend the atomic product-create RPC (POST /api/products fast path) with
-- the two new optional price columns.
