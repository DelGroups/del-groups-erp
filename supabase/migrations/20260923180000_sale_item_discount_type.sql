-- Dual-mode line discount on Sales invoices: discount_percent now holds
-- either a percent (0-100) or a flat AZN-line amount, depending on the new
-- discount_type column. AR/GL are unaffected — sales.discount_total /
-- total_amount are still computed client-side (now discount-type-aware,
-- see calcLineTotal/calcDiscountTotal in database.types.ts) and persisted
-- verbatim by this RPC exactly as before.

ALTER TABLE sale_items
  ADD COLUMN IF NOT EXISTS discount_type TEXT NOT NULL DEFAULT 'percentage';

ALTER TABLE sale_items DROP CONSTRAINT IF EXISTS sale_items_discount_type_check;
ALTER TABLE sale_items ADD CONSTRAINT sale_items_discount_type_check
...
BEGIN
  ...
  LOOP
    INSERT INTO sale_items (
      ...
      quantity, unit, unit_price, discount_percent, discount_type, vat_rate, line_total, extra_info,
    ...
    VALUES (
      ...
      CASE
        WHEN COALESCE(NULLIF(trim(v_item->>'discount_type'), ''), 'percentage') = 'fixed' THEN 'fixed'
        ELSE 'percentage'
      END,
